...
const assign = (category, image) =>
  `/images/${category}/${category} (${image}).jpg`;
...
export default function insertMongodb() {
  ...
  const correctedProducts = products.map((item) => {
    return {
      ...item,
      image: assign(item.category, item.image),
      price: parseInt(item.price),
      countInStock: parseInt(item.countInStock),
      ...(item.auxillaryImages && {
        auxillaryImages: item.auxillaryImages.map((auxImg) =>
          assign(item.category, auxImg)
        ),
      }),
    };
  });
  ...
  return (
    <button
      onClick={() => {
        axios.post("http://localhost:3000/api/products/", correctedProducts);
      }}
    ></button>
  );
}
